...
var WidgetGoogleMapHandler=function($scope,$){var googlemap_elem=$scope.find('.htmega-google-map').eq(0);if(googlemap_elem.length>0){var mapsettings=googlemap_elem.data('mapmarkers');var mapsoptions=googlemap_elem.data('mapoptions');var mapstyles=googlemap_elem.data('mapstyle');if(mapsettings&&Array.isArray(mapsettings)){mapsettings=mapsettings.map(function(marker){if(marker.baloon_text){var $temp=$('<div></div>');$temp.html(marker.baloon_text);$temp.find('script,iframe,object,embed,form,input,style,meta,link,base,applet').remove();$temp.find('*').removeAttr('onclick onload onerror onmouseover onmouseout onmouseenter onmouseleave');marker.baloon_text=$temp.html()}
return marker})}
...
var WidgetNotifyHandler=function($scope,$){var notify_elem=$scope.find('.htmega_notify_area').eq(0);var notify_opt=notify_elem.data('notifyopt');if(notify_elem.length>0){$(notify_opt.notify_btn_class).on("click",function(){var $temp=$('<div></div>');$temp.html(notify_opt.notifymessage);$temp.find('*').removeAttr('onclick onload onerror onmouseover onmouseout onmouseenter onmouseleave');$temp.find('script,iframe,object,embed,form,input,style,meta,link,base,applet').remove();var message=$temp.html();$.notify({},{type:notify_opt.type,element:notify_opt.notify_class,delay:notify_opt.delay,timer:1000,newest_on_top:!0,mouse_over:null,animate:{enter:'animated '+notify_opt.enter,exit:'animated '+notify_opt.exit},placement:{from:notify_opt.from,align:notify_opt.align},offset:notify_opt.offset,spacing:10,z_index:99999,template:'<div class="htmega-alert-wrap-'+notify_opt.wrapid+' '+notify_opt.width+' alert alert-{0}">'+'<span data-notify="dismiss" class="htmega-close"><i class="fas fa-times"></i></span>'+notify_opt.icon+'<span class="notify-message-content">'+message+'</span></div>'})})}}
